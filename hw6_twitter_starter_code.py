...
def make_request_with_cache(baseurl, hashtag, count):
    ...
    if isinstance(hashtag,str):
        hashtag={'q':hashtag,'count':count}
    key=construct_unique_key(baseurl,hashtag)
    CACHE_DICTION=open_cache()
    if key in CACHE_DICTION:
        print ("fetching cached data")
        result=CACHE_DICTION[key]
    else:
        print("making new request")
        result=make_request(baseurl,hashtag)
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    for i in tweet_data['statuses']:
        for x in i['entities']['hashtags']:
            text=x['text'].lower()
            if text==hashtag_to_ignore[1:].lower():
                pass
            elif text in common:
